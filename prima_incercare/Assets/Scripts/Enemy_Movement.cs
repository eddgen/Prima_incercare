...
public class Enemy_Movement : MonoBehaviour
{
    ...
    private void CheckForPlayer()
    {
        ...
        if (hits.Length > 0)
        {
            ...
            //if the player in in attack range and cooldown is ready
            if (UnityEngine.Vector2.Distance(transform.position, player.transform.position) <= attackingRange && attackCooldownTimer <= 0)
            {
                attackCooldownTimer = atttackCooldown;
                ChangeState(EnemyState.Attacking);
            }
            else if (UnityEngine.Vector2.Distance(transform.position, player.position) > attackingRange)
            {
